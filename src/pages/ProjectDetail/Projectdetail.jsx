import React, { useRef, useState, useEffect } from 'react';
import { IoPlayOutline } from "react-icons/io5";
import { RiPlayReverseLargeLine } from "react-icons/ri";
import { IoHeartSharp } from "react-icons/io5";
import { Swiper, SwiperSlide } from 'swiper/react';
import { TiMediaPlayReverseOutline } from "react-icons/ti";
import 'swiper/css';
import { TiMediaPlayOutline } from "react-icons/ti";
import { SlControlPlay } from "react-icons/sl";
import { TiStarFullOutline } from "react-icons/ti";
import { IoMdHeart } from "react-icons/io";
import component from '../../assets/Component 5.svg';
import Mavis from '../../assets/ec92218008540f58212964c427891bf8327f0d27.png';
import smile from '../../assets/img/Smile_rating.svg';
// import './style.scss'
import { FaBasketShopping } from "react-icons/fa6";
import Mentor from '../../components/Mentor/Mentor';
import Courses1detail from '../../components/Courses1/Courses1detail/Courses1detail';
import OtherMentors from '../../components/Mentor/otherMentors/OtherMentors';
import avatar from '../../assets/Avatar.svg'
import { IoMdShare } from "react-icons/io";
import mack from '../../assets/MacBook Pro.png'
import marina from '../../assets/Frame 1261154916.svg'
import OtherProject from '../../components/Projects/otherproject/OtherProject';
const ProjectDetail = () => {
  const swiperRef = useRef(null);
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    const handleResize = () => {
      setIsMobile(window.innerWidth <= 1024);
    };
    handleResize();
    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, []);
  const [currentSlide, setCurrentSlide] = useState(0);
  
    const goToSlide = (index) => {
      swiperRef.current.swiper.slideTo(index);
    };

const otzyv = [
  {
    id: 1,
    name: "Канатова Рано ",
    title: 'Несмотря на то, что курс онлайн, преподаватели всегда были на связи, оперативно отвечали на вопросы и давали развернутую обратную связь по заданиям. Это очень мотивировало',
    rating: 5.0,
    img: 'data:image/png;base64,'
  },
  {
    id: 2,
    name: "Канатова Рано ",
    title: 'Несмотря на то, что курс онлайн, преподаватели всегда были на связи, оперативно отвечали на вопросы и давали развернутую обратную связь по заданиям. Это очень мотивировало',
    rating: 5.0,
     img: 'data:image/png;base64,'
  },
  {
    id: 3,
    name: "Канатова Рано ",
    title: 'Несмотря на то, что курс онлайн, преподаватели всегда были на связи, оперативно отвечали на вопросы и давали развернутую обратную связь по заданиям. Это очень мотивировало',
    rating: 5.0,
     img: 'data:image/png;base64,'
  },
  {
    id: 4,
    name: "Канатова Рано ",
    title: 'Несмотря на то, что курс онлайн, преподаватели всегда были на связи, оперативно отвечали на вопросы и давали развернутую обратную связь по заданиям. Это очень мотивировало',
    rating: 5.0,
    img: 'data:image/png;base64,'
  },
  {
    id: 5,
    name: "Канатова Рано ",
    title: 'Несмотря на то, что курс онлайн, преподаватели всегда были на связи, оперативно отвечали на вопросы и давали развернутую обратную связь по заданиям. Это очень мотивировало',
    rating: 5.0,
     img: 'data:image/png;base64,'
  },
  {
    id: 6,
    name: "Канатова Рано ",
    title: 'Несмотря на то, что курс онлайн, преподаватели всегда были на связи, оперативно отвечали на вопросы и давали развернутую обратную связь по заданиям. Это очень мотивировало',
    rating: 5.0,
     img: 'data:image/png;base64,'
  },
]
 

  return (
   <div className=''>
  <div className='container flex w-full gap-2 justify-start items-center'>
    <h1 className='flex gap-[20px] font-bold text-[20px] justify-center items-center'>
      <TiMediaPlayReverseOutline />Назад
    </h1>
    <h1 className='flex justify-center items-center ml-[30px] text-[16px] font-[500] text-[#CFCFCF] gap-[10px]'>
      Главный <TiMediaPlayOutline />
    </h1>
    <h2 className='ml-[10px] font-[500] text-[16px]'>Проект</h2>
  </div>

  <div className='container !mt-[60px] w-full flex flex-wrap gap-x-[32px] gap-y-[40px]'>
    {/* <div className='w-[571px] h-[342px] bg-[#FFCD66] rounded-[24px] relative'>
      <img src={Mavis} alt="Mavis" className='w-full h-full rounded-[24px]' />
      <button className='absolute top-[129px] left-[244px] w-[84px] h-[84px] rounded-[51px] bg-white flex justify-center items-center text-[26px]'>
        <SlControlPlay className='text-[#FFD02C] ml-[5px]' />
      </button>
    </div> */}

    <div className='w-full overflow-hidden  relative h-[395px] px-[30px] py-[35px] bg-[#2D2D2D] rounded-[24px] leading-[55px] text-white'>
      <h1 className='font-bold text-[32px] font-[700] text-white'>Проект Mind Mentor</h1>
        <div className='flex'>
          <p className='flex justify-center items-center gap-[10px] text-[18px] font-[700]'>
            4.5 <TiStarFullOutline className='text-[25px]' />
          </p>
          <span className='text-[#6F6D73] ml-[10px] text-[14px] font-[500]'>445 оценок</span>
        </div>
      <div className='flex w-[540px] justify-between items-center'>
        <div className='text-[#6F6D73] w-full justify-between flex gap-[20px] text-[14px] font-[500]'>
          <p>Последнее обновление: 09.2024</p>
          <p className='ml-[80px]'>Нужны 3 специалиста</p>
          <p>Русский</p>
        </div>
      </div>
        <div className='w-[540px] flex justify-between text-[16px] font-[600] '>
     <h2>Проект менеджер:</h2>
     <h2 className='flex gap-[10px]'>Марина Сополова <img src={avatar} alt="" /></h2>
        </div>
      <h3 className='text-[24px] font-bold leading-[140%]'>от 2000 С</h3>
      <div className='flex justify-between items-center w-[540px] mt-[20px]'>
        <button className='w-[380px] h-[64px] bg-white rounded-[24px] text-[20px] font-[700] text-black'>
          Подать заявку
        </button>
        <button className='w-[64px] h-[64px] flex justify-center items-center text-[24px]'>
          <IoMdHeart />
        </button>
        <button className='w-[64px] h-[64px] flex justify-center items-center text-[24px]'>
          <IoMdShare />
        </button>
      </div>
      <div className='p-[30px] w-[500px] absolute h-[500px] top-[92px] right-[30px] bg-[#E1F3FF] rounded-tl-full rounded-tr-full rounded-bl-none rounded-br-none'>
        <img src={mack} alt="" />
      </div>
    </div>

    <div className='w-[574px] h-[468px]'>
      <h1 className='text-[24px] font-[700]'>Нужные специалисты</h1>
      <div className='flex flex-wrap mt-[20px] gap-[20px]'>
        <div className='px-[20px] rounded-[20px] border-1 border-black  py-[10px] text-[16px] font-[600]'>
         Дизайнеры
        </div>
        <div className='px-[20px] rounded-[20px] border-1 border-[#FFD02C]  py-[10px] text-[16px] font-[600]'>
          Frontend Разработчики
        </div>
        <div className='px-[20px] rounded-[20px] border-1 border-[#23AFCE]  py-[10px] text-[16px] font-[600]'>
          Backend Разработчики
        </div>
      </div>
      <h1 className='mt-[20px] text-[18px] font-[700]'>Наша команда</h1>
      <div className='flex gap-[10px] justify-between items-center mt-[20px]'>
        <div className='flex gap-[10px]'>
        <img src={marina} alt="Component" />
        <div className='flex flex-col justify-between items-start'>
        <p className='text-[16px] font-[500]'>Марина Сополова</p>
         <p className='text-[16px] font-[500] text-[#6F6D73]'>2 года опыта</p>
        </div>
        </div>
        <div className='text-[16px] font-[600] text-[#6F6D73]'>
          <p>Проект менеджер</p>
        </div>
      </div>
      <div className='flex gap-[10px] justify-between items-center mt-[20px]'>
        <div className='flex gap-[10px]'>
        <img src={marina} alt="Component" />
        <div className='flex flex-col justify-between items-start'>
        <p className='text-[16px] font-[500]'>Марина Сополова</p>
         <p className='text-[16px] font-[500] text-[#6F6D73]'>2 года опыта</p>
        </div>
        </div>
        <div className='text-[16px] font-[600] text-[#6F6D73]'>
          <p>Frontend разработчик</p>
        </div>
      </div>
      <div className='flex gap-[10px] justify-between items-center mt-[20px]'>
        <div className='flex gap-[10px]'>
        <img src={marina} alt="Component" />
        <div className='flex flex-col justify-between items-start'>
        <p className='text-[16px] font-[500]'>Марина Сополова</p>
         <p className='text-[16px] font-[500] text-[#6F6D73]'>2 года опыта</p>
        </div>
        </div>
        <div className='text-[16px] font-[600] text-[#6F6D73]'>
          <p>Backend разработчик</p>
        </div>
      </div>
     
     
    </div>

    <div className='w-[574px] h-[368px]'>
      <h1 className='text-[24px] font-[700] mb-[20px]'>Требование к специалистам</h1>
      <p className='mt-[30px] text-[16px] font-[500]'>Нет</p>
      <p className='mt-[30px] text-[24px] font-[700]'>Описание</p>
      <p className='text-[16px] font-[400] leading-[25px] mt-[30px]'>
       Вы узнаете в этом курсы,как создавать сайты с нуля.Научитесь работать програмами как Figma,Affter Effect.Сделаем 3 полноценных проекта для вашего Вы узнаете в этом курсы,как создавать сайты с нуля.Научитесь работать програмами.Сделаем 3 полноценных проекта для вашего Вы узнаете в этом курсы,как создавать сайты с нуля.Научитесь работать програмами как.Сделаем 3 полноценных проекта для вашего Вы узнаете в этом курсы,как создавать сайты с нуля. Научитесь работать програмами как.Сделаем 3 полноценных проекта для вашегоВы узнаете в этом курсы,как создавать сайты с нуля.Научитесь работать програмами.Сделаем 3 полноценных проекта для вашего Вы узнаете в этом курсы,как создавать сайты с нуля. Научитесь работать програмами как.Сделаем 3 полноценных проекта для вашего Вы узнаете в этом курсы,как создавать сайты с нуля.Научитесь работать програмами.
      </p>
    </div>
  </div>

  <div className=' m-auto'>
    <OtherProject/>
  </div>
  <div className='container'>
      <h1 className='text-[24px] text-[#000000] font-[700]'>Отзывы</h1>
      <Swiper
        spaceBetween={30} // Расстояние между слайдами
        slidesPerView={1} // Один слайд за раз
        loop // Бесконечный цикл
        ref={swiperRef} // Ссылка на Swiper
        onSlideChange={(swiper) => setCurrentSlide(swiper.activeIndex)} // Обновляем индекс текущего слайда
      >
        {/* Покажем 3 комментария в одном слайде */}
        <SwiperSlide>
          <div className="flex flex-col gap-4">
            {otzyv.slice(0, 3).map((item) => (
              <div key={item.id} className="flex items-center gap-4">
                <div className="w-[66px] flex flex-col justify-center items-center">
                  <img
                    src={item.img}
                    alt=""
                    className="w-[50px] h-[50px] rounded-full"
                  />
                  <p className="text-[12px] font-[700]">{item.rating}.0 😊</p>
                </div>
                <div className="w-full ml-[20px]">
                  <h1 className="text-[20px] font-[500] text-[#0F5257]">{item.name}</h1>
                  <p className="text-[16px] font-[500] leading-[20px]">{item.title}</p>
                </div>
              </div>
            ))}
          </div>
        </SwiperSlide>

        {/* Покажем следующие два комментария в другом слайде */}
        <SwiperSlide>
          <div className="flex flex-col gap-4">
            {otzyv.slice(3).map((item) => (
              <div key={item.id} className="flex items-center gap-4">
                <div className="w-[66px] flex flex-col justify-center items-center">
                  <img
                    src={item.img}
                    alt=""
                    className="w-[50px] h-[50px] rounded-full"
                  />
                  <p className="text-[12px] font-[700]">{item.rating}.0 😊</p>
                </div>
                <div className="w-full ml-[20px]">
                  <h1 className="text-[20px] font-[500] text-[#0F5257]">{item.name}</h1>
                  <p className="text-[16px] font-[500] leading-[20px]">{item.title}</p>
                </div>
              </div>
            ))}
          </div>
        </SwiperSlide>
      </Swiper>
      <div className="flex items-center justify-between">
        {/* Кастомные кнопки для переключения слайдов и цифры */}
        <div className="flex items-center gap-4">
          {/* Нумерация слайдов */}
          <div className="flex gap-2">
            {otzyv.slice(0, 4).map((item, index) => (
              <button
                key={item.id}
                onClick={() => goToSlide(index)} // Переключение на слайд по индексу
                className={`text-[16px] w-[62px] h-[42px] rounded-[32px] bg-[#FAFAFA] font-semibold ${currentSlide === index ? 'text-[#2D2D2D]' : 'text-[#000000]'} hover:text-[#2D2D2D]`}
              >
                {index + 1}
              </button>
            ))}
          </div>
  <button
            onClick={() => swiperRef.current.swiper.slidePrev()} // Переключение на предыдущий слайд
            className="px-3 py-1 flex justify-center items-center  rounded-[32px] w-[62px] h-[42px] bg-[#2D2D2D] text-white shadow-lg"
          >
            <RiPlayReverseLargeLine />
          </button>
          <button
            onClick={() => swiperRef.current.swiper.slideNext()} // Переключение на следующий слайд
            className="px-3 flex justify-center items-center py-1 rounded-[32px] w-[62px] h-[42px] bg-[#2D2D2D] text-white"
          >
               <IoPlayOutline />
          </button>
        </div>

        <div>
          <button className="w-[380px] h-[64px] bg-[#2D2D2D] text-white rounded-full font-semibold">
            Показать еще
          </button>
        </div>
      </div>
    </div>
</div>

  );
};

export default ProjectDetail;
