    ...
    const notifications = [
        {
            ...
            description: 'К сожалению мы не смогли получить оплату и мы отменили ваш курс "Курс по UX/UI - Марина Соколова"',
            date: "12 ноября",
            time: "12:00",
        },
        {
        ...
        },
        {
            ...
            type: "success",
            title: "Успешно!",
            description: '"Курс по UX/UI - Марина Соколова" Успешно добавлен в ваш профиль',
            date: "12 ноября",
            time: "12:00",
        },
    ]
        ...
        return (
            <>
                <div className="flex items-center justify-between p-5">
                    ...
                    <img className="absolute top-8 right-10 text-xl font-bold text-black hover:text-gray-600" src={x11}
                         alt=""
                         onClick={closePopup}
                    />
                </div>
                ...
                <div className="flex px-5 mb-5">
                    ...
                    <button
                        ...
                        style={{borderRadius: "10px", backgroundColor: 'rgba(250, 250, 250, 1)'}}
                ...
                <div className="px-5 pb-5">
                    {messages.map((message) => (
                        <div key={message.id} className="bg-gray-50 rounded-xl p-4 mb-4" style={{ borderRadius: "16px" }}>
                            ...
                            <div className="flex justify-between items-center">
                                <p className="text-gray-700 text-base font-normal">{message.title}</p>
            ...
            <>
                ...
                <div className="px-5 pb-5">
                    {notifications.map((notification) => (
                        <div key={notification.id} className="bg-gray-50 rounded-xl p-4 mb-3">
                            <h3 className={notification.type === "error" ? "text-red-500" : "text-cyan-500"}>{notification.title}</h3>
        ...
        <header className="w-full bg-white px-4 py-3 mt-[15px] relative">
            <div className="container mx-auto flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <button className="lg:hidden block mr-2" onClick={() => setIsOpen(!isOpen)} aria-label="Toggle menu">
                        {isOpen ? <X size={24} /> : <Menu size={24} />}
                    </button>
                    <img src={md || "/placeholder.svg"} alt="логотип" className="h-[120px] w-[170px]" />
                </div>
            ...
            <div className={`lg:hidden ${isOpen ? "block" : "hidden"} absolute top-full left-0 w-full bg-white py-4 z-10`}>
            ...
            {showPopup && (
                <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 px-4 sm:px-0">
                    <div className="absolute inset-0 bg-black/30" onClick={closePopup}></div>
                    <div
                        className="relative bg-white shadow-lg w-full max-w-[400px] mx-auto overflow-hidden"
                        style={{ borderRadius: "24px" }}
                    >
                        {activeView === "messages" ? renderMessagesView() : renderNotificationsView()}
                    </div>
                </div>
            )}
        </header>
